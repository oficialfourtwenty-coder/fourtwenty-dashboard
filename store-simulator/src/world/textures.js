...
// Piso panal gris de vereda Burela: juntas oscuras, desgaste y variacion leve.
export function burelaHexPaver(repeatX, repeatY) {
  const c = makeCanvas();
  const ctx = c.getContext('2d');
  ...
  for (let row = -1, y = 0; y < SIZE + R; row++, y = row * h) {
    ...
    for (let x = -w; x < SIZE + w; x += w) {
      ...
      ctx.fillStyle = `rgb(${154 + t},${152 + t},${143 + t})`;
      hexPath(x + xoff, y);
      ctx.fill();
    }
  }
  noise(ctx, 0, 0.08, 1300);
  for (let i = 0; i < 8; i++) {
    ctx.fillStyle = 'rgba(80,76,68,0.16)';
    ctx.beginPath();
    ctx.ellipse(Math.random() * SIZE, Math.random() * SIZE, 16 + Math.random() * 28, 8 + Math.random() * 18, Math.random() * 3, 0, Math.PI * 2);
    ctx.fill();
  }
  return toTexture(c, repeatX, repeatY);
}

// Intrados acanalado de la marquesina: lineas finas paralelas, off-white.
export function ribbedCeiling(repeatX, repeatY) {
  const c = makeCanvas();
  const ctx = c.getContext('2d');
  ctx.fillStyle = '#CDCBC2';
  ctx.fillRect(0, 0, SIZE, SIZE);
  noise(ctx, 0, 0.04, 700);
  ctx.strokeStyle = 'rgba(76,72,66,0.28)';
  ctx.lineWidth = 2;
...
// Ladrillo visto rojo apagado con mortero claro.
export function brickWall(repeatX, repeatY) {
  const c = makeCanvas();
  const ctx = c.getContext('2d');
  ...
  for (let y = 0; y < SIZE + brickH; y += brickH) {
    ...
    for (let x = offset; x < SIZE + brickW; x += brickW) {
      const t = (Math.random() - 0.5) * 24;
      ctx.fillStyle = `rgb(${158 + t},${77 + t * 0.4},${58 + t * 0.35})`;
